...
namespace irgen
{
    ...
    ir_generator::var_from_node(const node::node& root)
    {
        ...
        if (dtype == icode::STRUCT)
        {
            ...
            {
                miklog::error_tok(
                  module.name, "Symbol does not exist", file, dtype_token);
                throw miklog::compile_error();
            }
    ...
    void ir_generator::use(const node::node& root)
    {
        for (node::node child : root.children)
        {
            ...
            {
                miklog::error_tok(
                  module.name, "Self import not allowed", file, name_token);
                throw miklog::compile_error();
            }
    ...
    void ir_generator::from(const node::node& root)
    {
        ...
        for (node::node child : root.children[1].children)
        {
            ...
            {
                miklog::error_tok(
                  module.name, "Cannot import functions", file, child.tok);
                throw miklog::compile_error();
            }
            /* If is a def */
    ...
    void ir_generator::assign_str_literal_tovar(op_var_pair var, node::node& root)
    {
        ...
        for (size_t i = 0; i < str_int.size(); i++)
        {
            ...
            builder.copy(curr_offset,
                         icode::literal_opr(target.default_int, character, id()));
    ...
    void ir_generator::assign_init_list_tovar(op_var_pair var, node::node& root)
    {
        ...
        for (size_t i = 0; i < root.children.size(); i++)
        {
            ...
            if (element_var.dimensions.size() == 0)
            {
                ...
                {
                    miklog::error_tok(
                      module.name, "Incorrect dimensions", file, child.tok);
                    throw miklog::compile_error();
                }
    ...
    op_var_pair ir_generator::size_of(const node::node& root)
    {
        ...
        while (mod_node.type == node::MODULE)
        {
            ...
            {
                miklog::error_tok(
                  module.name, "Module does not exist", file, mod_node.tok);
                throw miklog::compile_error();
            }
        ...
        return op_var_pair(icode::literal_opr(icode::INT, size, id()),
    ...
    op_var_pair ir_generator::term(const node::node& root)
    {
        ...
        switch (child.type)
        {
            ...
            case node::MODULE:
            {
                ...
                {
                    miklog::error_tok(
                      module.name, "Module does not exist", file, child.tok);
                    throw miklog::compile_error();
                }
    ...
    void ir_generator::print(const node::node& root)
    {
        for (size_t i = 0; i < root.children.size(); i++)
        {
            ...
            if (child.type == node::STR_LITERAL)
            {
                ...
                builder.printop(icode::PRINT_STR,
                                gen_str_dat(child.tok, print_var).first);
            }
            /* Else expression */
            else
            {
    ...
    void ir_generator::input(const node::node& root)
    {
        ...
        if (input_var.second.dimensions.size() == 0)
            ...
            builder.inputop(
              icode::INPUT_STR, input_var.first, input_var.second.dimensions[0]);
    }
        ...
        for (node::node stmt : root.children)
        {
            switch (stmt.type)
            {
                ...
                case node::BREAK:
                {
                    ...
                    {
                        miklog::error_tok(
                          module.name, "BREAK outside loop", file, stmt.tok);
                        throw miklog::compile_error();
                    }
