...
+ Create debug build
  ...
  ```
  make build DEBUG=1 CXX=ccache\ g++ -j 4
  ```
...
+ Create coverage build
  ...
  ```
  make build GCOV=1 CXX=ccache\ g++ -j 4
  ```

+ Run coverage, make sure it passes all tests
  ```
  make coverage
  ```

+ Review changes using git diff (or other GUI tools), especially the test cases
